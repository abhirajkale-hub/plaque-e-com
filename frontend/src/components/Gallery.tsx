...
const defaultImages: DisplayImage[] = [
  {
    src: gallery1,
...
export const Gallery = () => {
  const [images, setImages] = useState<DisplayImage[]>(defaultImages);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadImages = async () => {
      try {
        const galleryImages = await uploadService.getGalleryImages();
        if (galleryImages && galleryImages.length > 0) {
          // Map gallery images to display format
          const mappedImages: DisplayImage[] = galleryImages.map((img) => ({
            // Use the backend URL to serve the uploaded image
            src: `${BACKEND_BASE_URL}${img.imageUrl}`,
            alt: img.description || img.title,
            customer: img.customerName || "Valued Customer",
            role: img.customerRole || "Funded Trader",
          }));
          setImages(mappedImages);
        } else {
          // Use default images if no gallery images
      ...
      <div className="container px-4 sm:px-6">
        ...
        <div className="max-w-3xl mx-auto text-center mb-12 sm:mb-16 animate-fade-in">
          ...
          <p className="text-base sm:text-xl text-muted-foreground px-4">
            See how funded traders celebrate their milestones with My Trade
            Award
          </p>
        </div>
              ...
              >
                ...
                <div className="aspect-square overflow-hidden">
                  <img
                    ...
                    loading="lazy"
                    onError={(e) => {
                      console.warn("Failed to load uploaded image:", image.src);
                      e.currentTarget.src =
                        "https://placehold.co/400x400?text=Trader+Setup";
                    }}
                  />
                </div>
          ...
          <div className="flex items-center justify-center gap-2">
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star
                  key={star}
                  className="w-5 sm:w-6 h-5 sm:h-6 fill-primary text-primary"
                />
              ))}
            </div>
